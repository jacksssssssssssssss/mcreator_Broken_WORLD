...
import net.mcreator.brokensmpgodshards.procedures.BoltProjectileHitsBlockProcedure;
...
public class BoltEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(BrokenSmpGodShardsModItems.BULLET.get());

	public BoltEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(BrokenSmpGodShardsModEntities.BOLT.get(), world);
	}
	...
	public void onHitBlock(BlockHitResult blockHitResult) {
		...
		BoltProjectileHitsBlockProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}
	...
	public static BoltEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		BoltEntity entityarrow = new BoltEntity(BrokenSmpGodShardsModEntities.BOLT.get(), entity, world);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.amethyst_block.chime")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static BoltEntity shoot(LivingEntity entity, LivingEntity target) {
		BoltEntity entityarrow = new BoltEntity(BrokenSmpGodShardsModEntities.BOLT.get(), entity, entity.level());
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.amethyst_block.chime")), SoundSource.PLAYERS, 1,
